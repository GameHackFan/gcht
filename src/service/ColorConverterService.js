...
class ColorConverterService
{
	...
	getColorHex32 = (colorData, channelOrderKey, channelSizeKey) =>
	{
		...
		if(colorConverterService.isValidColorMainData(cmd))
		{
			...
			append = isNaN(append) ? 0 : append;
			da = isNaN(parseInt(da)) ? 0 : da + append;
	...
	convertHexToAnyBitsColor = (hex, channelIndexes, channelSize) =>
	{
		if(hex)
		{
			...
			return convertDecimalToAnyBitsColor(d, channelIndexes, channelSize);
		}
	...
	{
		return [this.getValidChannelValue(red),
...
let colorConverterService = new ColorConverterService();
export default colorConverterService;